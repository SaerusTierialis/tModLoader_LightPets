...
namespace LightPets.Buffs
{
    ...
    public class LanternSpirit : ModBuff
    {
        ...
        public override void Update(Player player, ref int buffIndex)
        {
            ...
            int projID = mod.ProjectileType(GetType().Name);
            bool petProjectileNotSpawned = player.ownedProjectileCounts[projID] <= 0;
            if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
            {
                Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, projID, 0, 0f, player.whoAmI, 0f, 0f);
            }
        }
    }
}